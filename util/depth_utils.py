...
class ConvBlock_Ch(nn.Module):
    """Layer to perform a convolution followed by ELU
    """

    def __init__(self, in_channels, out_channels):
        super(ConvBlock_Ch, self).__init__()
        self.reduction_ratio = 2
        self.middle_layer_size = int(in_channels / float(self.reduction_ratio))

        self.bottleneck = nn.Sequential(
            nn.Linear(in_channels, self.middle_layer_size),
            nn.ELU(inplace=True),
            nn.Linear(self.middle_layer_size, out_channels)
        )
        # self.conv = Conv3x3(in_channels, out_channels)
        # self.nonlin = nn.ELU(inplace=True)
